...
11. **Transferred-namespace tombstone.** The framework repo moved from `allendosapes/` to `dos-apes/` on 2026-08-08. GitHub's redirect keeps old URLs working, but creating ANY repo or fork named `dos-apes-super-agent-framework` under the `allendosapes` account permanently and irreversibly deletes that redirect. Never recreate that name under the personal account.

12. **Unfetched origin state is unverified.** Claims about origin (branch lists, "up to date", ahead/behind counts) are invalid unless a `git fetch` ran first in the same session. During the org migration a stale clone reported "up to date" while 13 commits behind, and reported 4 remote branches when origin had 10 — and the wrong count reached a verification report formatted as evidence. Same defect class as session-cwd branch checks in `guard-main-branch.sh` (#8): cached state presented as target state. Always fetch before asserting.

13. **Merge detection is structurally broken under squash + linear history.** This repo's `Main` ruleset requires linear history and merges are squash-only. A squash rewrites branch commits into one new commit with a different SHA and a different patch-id, so no branch tip is ever an ancestor of `main`, and the squash's patch-id differs from those of the commits it replaced except in the single-commit case described below. Consequence: `git branch -r --merged main` is always wrong here — the squash always carries a new SHA, so the branch tip is never an ancestor of `main`. `git cherry -v main <branch>` is wrong for any branch with more than one commit; it happens to be correct when a squash collapses exactly one commit, because a single-commit squash preserves the patch-id. That exception is not usable — it depends on a property of the branch you'd have to check first, and the tool gives no signal when it doesn't hold. Treat both as unusable for merge status in this repo, without exception. Determine merge status by content instead: the PR's merge commit SHA via `gh pr list --state merged`, plus tree-level verification that the expected content is present on `main`.
