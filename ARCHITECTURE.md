...
|---------|------|---------|
| 3.6.0 | 2026-08 | Permission-hardening release (M-0001 through M-0004), shipping on the beta channel with `latest` pinned at 3.0.0. M-0001 replaced the inverted v1 permission template with a least-privilege deny→ask→allow policy plus a PreToolUse guard hook. M-0002 scoped `allowed-tools` frontmatter across all 18 commands and 15 skills and added the L1 drift-guard (`allowed-tools-guard.test.js`). M-0003 rewrote the bodies underneath: a shipped deny-collision fixed, 18 inline `node -e` segments replaced by six named helper scripts, 11 `git checkout` sites migrated to `git switch`, bare-form and env-prefix sweeps across 9 sites. M-0004 fixed the main-branch guard to resolve state from the target path's worktree rather than the session cwd, hardened `mission-cli move` for untracked companion files, and added a reverse packaging assertion. |
| 3.5.1 | 2026-05 | L8 hardening patch. Five defects in the Codex cross-model review pipeline plus two YAML-authoring papercuts. Notably: the OpenAI structured-output schema was rejected with HTTP 400 because `required` must be a superset of `properties` keys (optionality is a `["T","null"]` union), which had blocked 100% of L8 reviews; and `PROJECT_ROOT` derived from `process.cwd()` resolved `.dos-apes/` against the worktree rather than the main checkout, now resolved via `git rev-parse --git-common-dir`. No API changes. |
